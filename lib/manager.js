...
if (cluster.isMaster) {
  ...
  if(config.task_list.length == 0) {
    console.log("Error loading configuration file(s). No jobs found. Terminating.");
    process.exit();
  }
  ...
  for (var i = 0; i < agent_count; i++) {
    agents[i] = cluster.fork({ task_list: JSON.stringify(agent_task_lists[i]), hosts: argv.hosts, duration: argv.duration });

    agents[i].on('message', function(msg) {
      if(msg.statistic) {
        benchmark_stats.store_statistic(msg.statistic.job_id, msg.statistic.label, msg.statistic.result.result_code, 
  ...
  setTimeout(function send_heartbeat() {
    var heartbeat = {heartbeat: true };
    if(!terminating) {
      for (var i in agents) {
        agents[i].send(heartbeat);
      }

      setTimeout(send_heartbeat, 10000, agents);
    }   
}, 10000, agents); 
...
function log_stats(runtime, queue, stats_array) {
  for (var i = 0; i < stats_array.length; i++) {
    ...
    stat.latency_avg = s.mean;
    stat.latency_stddev = s.std_dev;
