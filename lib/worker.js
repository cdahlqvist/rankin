...
module.exports.run = function(hosts, task) {
  ...
  setTimeout(function benchmark(driver, esClient, state, task, operations, interval) {
    ...
    driver[op.name](esClient, state, function result_callback(result) {
      ...
      setTimeout(benchmark, delay, driver, esClient, state, task, operations, interval);
    });
  }, initial_delay, driver, esClient, state, task, operations, interval);
}
