...
module.exports.run = function(hosts, task, driver_data) {
  ...
  if(typeof driver.init === 'function') {
    state = driver.init(esClient, task.parameters, driver_data);
  } else {
    state = {};
  }
  ...
  setTimeout(function benchmark(driver, esClient, state, driver_data, task, operations, opindex, interval) {
    ...
    driver[op.name](esClient, state, driver_data, op.parameters, function result_callback(result) {
      ...
      var parameters = util.clone(task.parameters);
       
      for(var key in op.parameters) {
        if(util.is_string(key)) {
          parameters[key] = op.parameters[key];
        }
      }

      process.send( { statistic: { timestamp: util.get_timestamp_string(), job_id: task.job_id, driver: task.driver, record_type: "detail", operation: op.name, label: label, parameters: parameters, result: result, latency: latency, sla_breach: sla_breach} } );

      var delay = Math.max(0, (interval - (util.get_timestamp() - start_ts)));

      setTimeout(benchmark, delay, driver, esClient, state, driver_data, task, operations, opindex, interval);
    });
  }, initial_delay, driver, esClient, state, driver_data, task, operations, opindex, interval);
}
